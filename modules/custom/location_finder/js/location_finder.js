(function ($, window, Drupal, drupalSettings) {
  ...
  Drupal.location_finder = function() {
    return {
  ...
  Drupal.behaviors.location_finder = {};
  Drupal.behaviors.location_finder.attach = function (context) {
    $(context).find('body').once('locations').each(function () {
      // Get locations from HTML.
      var locations = {
        locations: []
      };
      $('.locations-list .node--view-mode-teaser').each(function() {
        var $self = $(this), data = $self.data('locations');
        data.element = $self.parent();
        data.markup = $self.get(0).outerHTML;
        locations.locations.push(data);
      });

      var map = new Drupal.location_finder();
      map.init({
        component_el: $('#location-finder'),
        map_data: locations
      });
    });
  };
