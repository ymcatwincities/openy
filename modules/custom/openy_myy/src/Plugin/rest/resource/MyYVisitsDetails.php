...
/**
 * Provides a Visits Details data
 *
 * @RestResource(
 *   id = "myy_profile_visits_details",
 *   label = @Translation("Stat of user visits"),
 *   uri_paths = {
 *     "canonical" = "/myy-model/data/profile/visits-details/{personify_ids}/{start}/{finish}"
 *   }
 * )
 */
class MyYVisitsDetails extends ResourceBase {
   ...
   * {@inheritdoc}
   */
  public function get($personify_ids, $start_date, $finish_date) {
    ...
    if (array_key_exists($myy_config['myy_data_visits'], $myy_authenticator_instances)) {
      $response = $this
        ...
        ->getVisitsDetails($personify_ids, $start_date, $finish_date);
    } else {
