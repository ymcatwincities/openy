...
/**
 * Form controller for the Member Registration popup form.
 *
...
class MemberRegisterForm extends FormBase {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if (!empty($storage['campaign'])) {
      ...
      // If a member ID is successfully registered during the registration phase
      if ($checkinsOpenDate >= new \DateTime()) {
        $date = $this->t('on @date', ['@date' => $checkinsOpenDate->format('F j')]);
        drupal_set_message(t('Thank you for registering, you are all set! Be sure to check back @date when the challenge starts!', ['@date' => $date]), 'status', TRUE);
      }
      // If Campaign already in checkings phase - login member
      $isCheckinsPeriod = $this->checkCheckinsPeriod($campaign);
      if ($isCheckinsPeriod) {
        // TODO Login member
        drupal_set_message(t('Thank you for registering, you are all set and logged in!'));
      }
    }
  }
