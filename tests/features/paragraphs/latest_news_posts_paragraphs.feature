@openy @api
Feature: Latest News Paragraphs
  Check that Latest News Posts paragraphs output other nodes based on some rules

  Background: Log in
    Given I am logged in as a user with the "Editor" role
    And I create taxonomy_term of type news_category:
      | name               |
      | BEHAT CATEGORY ONE |
    And I create paragraph of type latest_news_posts:
      | KEY          |
      | latest_news |
    And I create paragraph of type latest_news_posts_branch:
      | KEY            |
      | branch_news   |
      | branch_news_2 |
    And I create paragraph of type latest_news_posts_camp:
      | KEY        |
      | camp_news |
    And I create large branch content:
      ...
      | field_content                       | branch_news    | branch_news_2  |
    And I create large camp content:
      ...
      | field_content                       | camp_news         |
    And I create news content:
      | title                                              | field_news_location | promote | created           |
    ...
    Then I create landing_page content:
      ...
      | behat_landing_news | Behat Landing News | one_column      | latest_news  |

  Scenario: Paste latest news post
    Given I view node "behat_landing_news"
  ...
  Scenario: Paste latest news post (branch)
  ...
  Scenario: Paste latest news post for camp
