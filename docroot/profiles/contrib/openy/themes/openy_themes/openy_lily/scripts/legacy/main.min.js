...
(function($){
  ...
  init();
    ...
    (function () {
        ...
        nav.done(
            function () {
                // Uncomment next 2 lines if need to pass page related submenu to the sidebar.
...
//      });
  ...
  (function(){
    jQuery('.content-prgf-image .img-responsive').each(function () {
      var headerImgSrc = jQuery(this).attr('src');
      jQuery(this).addClass('visible-xs');
      if (!headerImgSrc) return;
      jQuery(this).parents('.content-prgf-image')
  ...
  (function()
    {
      if (jQuery('#slides-template').length === 0) {
        return;
      }
  ...
  function setJoinButton() {
    var jsonnav = CW.Navigation.JSON.get(CW.page.id),
      path = CW.page.liveUrl.slice(0, -1),
      page = CW.page.liveUrl.split('/').slice(1, -1),
      activeLocation,
      children = jQuery.extend(true, {}, jsonnav).children,
      isCamp = false,
      $joinBtn;
    ...
    function findLocation(children) {
      var childName, ii, l, iii, ll;
      for (var i = 0; i < children.length; i++) {
        if (children[i].magic_page == page[0] && children[i].children) {
          childName = children[i].page_name == 'Camps' ? 'Register' : 'Join';
          for (ii = 0, l = children[i].children.length; ii < l; ii++) {
            if (path.match(children[i].children[ii].url)) {
              for (iii = 0, ll = children[i].children[ii].children.length; iii < ll; iii++) {
                if (children[i].children[ii].children[iii].page_name == childName)
                  return children[i].children[ii].children[iii].url;
              }
            }
          }
        }
  ...
  function initSiteNav(){
    if (jQuery('#nav-item-template').length === 0) {
      return;
    }
