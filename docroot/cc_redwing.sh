...
if [ "$1" = "--windows" ]; then
    time ansible-playbook --extra-vars='is_windows=true site_url=redwing.192.168.56.132.xip.io staging="sites/ymca_redwing/config/staging"' ./config_clean.yml -i 'localhost,' --connection=local
else
    time ansible-playbook --extra-vars='site_url=redwing.192.168.56.132.xip.io staging="sites/ymca_redwing/config/staging"' ./config_clean.yml -i 'localhost,' --connection=local
fi
