...
class TangoCardTests extends PHPUnit_Framework_TestCase {
    ...
    public function testPlaceOrderForVariablePriceItem() {
        echo 'test Place order for variable price item  ';
        $response = $this->tango->placeOrder(self::$accountname, self::$accountid, 'Seattle', 'From', 'Subject of the message', 'message', 'TNGO-E-V-STD', 'name', 'a@a.com', TRUE, 1000);
        $this->assertFalse($response->success);
        var_dump($response);
    }

    public function testPlaceOrderForStaticPriceItem() {
        echo 'test Place order for static price item';
        $response = $this->tango->placeOrder(self::$accountname, self::$accountid, 'Seattle', 'From', 'Subject of the message', 'message', 'APPL-E-5000-STD', 'name', 'a@a.com', TRUE);
        $this->assertFalse($response->success);
        var_dump($response);
    }
